...
export default function AnalyticsChart({ campaigns }: { campaigns: ICampaign[] }) {
  const data = campaigns.map((campaign) => {
    ...
    const multiplier = 
      campaign.tier === "enterprise" ? 5 : 
      campaign.tier === "premium" ? 2.5 : 1;
    ...
    <div className="mb-8 h-80 w-full rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-800">
      ...
      <ResponsiveContainer width="100%" height="80%">
        ...
        >
          ...
          <Bar yAxisId="left" dataKey="budget" name="Budget (USD)" fill="#6366f1" radius={[4, 4, 0, 0]} maxBarSize={40} />
          <Bar yAxisId="right" dataKey="reach" name="Est. Reach" fill="#a855f7" radius={[4, 4, 0, 0]} maxBarSize={40} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
